...
2. `requirements.txt`의 의존성을 설치한 후 `python -m compileall src` 명령으로 소스가 정상적으로 컴파일되는지 확인합니다. (호환성을 위해 `requirment.txt`도 동일한 내용을 포함합니다.)
...
모델 추론 결과를 MLflow에 기록해 입력/출력과 캐시 사용 여부를 추적할 수 있습니다. 다음 환경 변수를 설정해 추적 서버 정보를 제공합니다.

| 환경 변수 | 설명 |
|-----------|------|
| `MLFLOW_TRACKING_URI` | MLflow 트래킹 서버 URI. 설정하지 않으면 로깅을 비활성화합니다. |
| `MLFLOW_EXPERIMENT_NAME` | (선택) 사용할 실험 이름. 기본값은 `wodfit-ml`입니다. |
| `MLFLOW_RUN_NAME` | (선택) 실행 이름. 기본값은 `wod-cluster-prediction`입니다. |

MLflow 로깅에서 추론 요청의 `wods`, `weights`, `predictions`가 JSON 아티팩트로 저장되며, 캐시 적중 여부 및 예측된 고유 클러스터 수가 함께 기록됩니다. MLflow 서버 연결에 실패해도 추론은 정상적으로 계속됩니다.
